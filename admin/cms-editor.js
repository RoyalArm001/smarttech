(function (window) {
  ...
  function saveActiveCollection(form) {
    ...
    try {
      if (state.activeId === "contacts") {
        ...
        payload = parseJsonInput(form.querySelector(".cms-json-editor").value, state.activeId === "services" || state.activeId === "projects" ? [] : {});
      }
    } catch (error) {
      setStatus(error.message, true);
      return;
  ...
  function loadCollection(id) {
    ...
    return requestJson("/api/admin/cms/" + encodeURIComponent(id)).then(function (payload) {
      ...
      state.data = payload.data || (id === "services" || id === "projects" ? [] : {});
      renderEditor();
    });
  }
